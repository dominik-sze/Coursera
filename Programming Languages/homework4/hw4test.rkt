...
(define tests
  (test-suite
   ...
   ; sequence test
   (check-equal? (sequence 1 0 1) (list ) "Sequence test")
   (check-equal? (sequence 0 5 1) (list 0 1 2 3 4 5) "Sequence test")
   (check-equal? (sequence 4 6 4) (list 4) "Sequence test")
   (check-equal? (sequence 4 12 4) (list 4 8 12) "Sequence test")

   ; string-append-map test
   (check-equal? (string-append-map (list ) ".jpg") '() "string-append-map test")
   ...
   (check-equal? (list-nth-mod (list 0 1 2) 5) 2 "list-nth-mod test")
   (check-equal? (list-nth-mod (list 3 1 0) 6) 3 "list-nth-mod test")
   (check-equal? (list-nth-mod (list 3 1 0) 11) 0 "list-nth-mod test")
   
   ; stream-for-n-steps test
   (check-equal? (stream-for-n-steps (lambda () (cons 1 ones)) 0) (list ) "stream-for-n-steps test")
   ...
   ; dan-then-dog test
   (check-equal? (stream-for-n-steps dan-then-dog 0) (list ) "dan-then-dog test")
   ...
   (check-equal? (stream-for-n-steps dan-then-dog 5) (list "dan.jpg" "dog.jpg" "dan.jpg" "dog.jpg" "dan.jpg") "dan-then-dog test")
   
   ; stream-add-zero test
   (check-equal? (stream-for-n-steps (stream-add-zero ones) 0) (list ) "stream-add-zero test")
   ...
   ; cycle-lists test
   (check-equal? (stream-for-n-steps (cycle-lists (list 1) (list "a" "b")) 0) (list ) 
                 "cycle-lists test")
   ...
   ; vector-assoc test
   (check-equal? (vector-assoc 1 (vector (cons 2 1) (cons 3 1) (cons 4 1) (cons 5 1))) #f "vector-assoc test")
   (check-equal? (vector-assoc 1 (vector 1 (cons 3 1) (cons 4 1) (cons 5 1))) #f "vector-assoc test")
   ...
   (check-equal? ((cached-assoc (list (cons 1 2) (cons 3 4)) 3) 6) #f "cached-assoc test")
